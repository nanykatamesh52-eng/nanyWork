...
class AI_Assistant:
    def __init__(self):
        ...
        # API setup
        # self.base_url = "http://37.224.62.239:89"
        # self.patient_base_url = "http://37.224.62.239:89"
        # self.common_headers = {
        #     "ProviderId": "5e1c1e4d-9049-483b-a03e-0225ed6a9223",
        #     "BranchId": "1",
        #     "org": "1",
        #     "Content-Type": "application/json",
        #     "Authorization": "Basic U2hvYWE6MTIz"
        # }
    ...
    4. يتم عرض تفاصيل قليله و لا يتم عرض كل الداتا
    ...
    4.show small details not all response of api 
    ...
    def get_clinics(self):
        """Get all clinics (only Code, TitleAr, TitleEn)"""
        ...
        try:
            ...
            # Safely extract only required fields
            clinics = []
            ...
            lines = [f"{i+1}. {c['TitleEn']}" for i, c in enumerate(clinics)]
            message_text = "\n".join(lines) + "\n\nPlease reply with the number of the clinic you'd like to choose!"

            return {"clinics": clinics, "message": message_text}
        ...
        try:
            ...
            for item in data.get("items", []):
                doctors.append({
                    ...
                    "TitleAr": item.get("TitleAr"),
                    "TitleEn": item.get("TitleEn"),
                    "Mobile": item.get("Details", {}).get("Mobile") if item.get("Details") else None
                })
            return doctors
    ...
    def check_patient_exists(self, mobile_number: str):
        url = f"{self.base_url}/api/wa/Lookup"
        payload = {"ContactMobile": mobile_number, "Type": "3"}
        ...
        try:
            ...
            if r.status_code == 200:
                try:
                    ...
                    if not items:
                        return {"success": True, "exists": False, "patients": [], "mobile_number": mobile_number}

                    if len(items) == 1:
                        return {"success": True, "exists": True, "multiple": False, "patients": items, "mobile_number": mobile_number}
                    else:
                        return {"success": True, "exists": True, "multiple": True, "patients": items, "mobile_number": mobile_number, "header_ar": response_data.get("ListHeaderTextAr"), "header_en": response_data.get("ListHeaderTextEn"), "footer_ar": response_data.get("ListFooterTextAr"), "footer_en": response_data.get("ListFooterTextEn")}
                except Exception as e:
                    return {"success": False, "error": f"Invalid response JSON: {str(e)}", "raw_response": r.text, "mobile_number": mobile_number}
            else:
                return {"success": False, "error": f"Failed to check patient. Status code: {r.status_code}", "response": r.text, "mobile_number": mobile_number}
        except Exception as e:
            return {"success": False, "error": f"Failed to check patient: {str(e)}", "mobile_number": mobile_number}

    def handle_patient_choice(self, choice_text):
        try:
            choice_num = int(choice_text.strip())
            if 1 <= choice_num <= len(self.patient_selection_state["patients"]):
                selected_patient = self.patient_selection_state["patients"][choice_num - 1]
                self.patient_selection_state["selected_patient"] = selected_patient
                self.patient_selection_state["awaiting_choice"] = False

                if self.current_language == "Arabic":
                    return f"✅ تم اختيار المريض: {selected_patient.get('TitleAr')} (الكود: {selected_patient.get('Code')}).\nيمكنك الآن كتابة 'عرض المواعيد' لعرض المواعيد أو 'حجز موعد جديد' للمتابعة."
                else:
                    return f"✅ Selected patient: {selected_patient.get('TitleEn')} (Code: {selected_patient.get('Code')}).\nYou can now type 'show appointments' to see appointments or 'book new appointment' to continue."
            else:
    ...
    def check_doctor_availability(self, doctor_code: str, date: str = None):
        ...
        if not date:
            date = datetime.now(pytz.timezone("Africa/Cairo")).strftime("%Y-%m-%d")
    ...
    def extract_date_from_text(self, text):
        ...
        if 'tomorrow' in text_lower or 'غدًا' in text_lower or 'غدا' in text_lower:
            tomorrow = datetime.now(pytz.timezone("Africa/Cairo")) + timedelta(days=1)
    ...
    def get_patient_appointments(self, mobile_number: str, patient_code: str = None):
        ...
        if not patient_check["exists"]:
            return {"success": True, "exists": False, "message": "Patient not found", "appointments": [], "mobile_number": mobile_number}

        # If multiple, filter by patient_code
        if patient_code:
            for p in patient_check.get("patients", []):
                if str(p.get("Code")) == str(patient_code):
                    return {"success": True, "exists": True, "appointments": p.get("Details", {}).get("UpcomingAppointments", []), "mobile_number": mobile_number, "patient_data": p}

        # Otherwise return first patient's appointments
        patient = patient_check["patients"][0]
        return {"success": True, "exists": True, "appointments": patient.get("Details", {}).get("UpcomingAppointments", []), "mobile_number": mobile_number, "patient_data": patient}

    # ------------------ AI Reasoning ------------------
    def generate_ai_response(self, text, language=None):
        """Send user text to OpenAI with tool calling"""
        # Patient selection step
        if self.patient_selection_state["awaiting_choice"]:
            response = self.handle_patient_choice(text)
            #self.generate_audio(response)
            return response

        # Direct commands after patient selection
        if text.strip() in ["عرض المواعيد", "show appointments"] and self.patient_selection_state.get("selected_patient"):
            patient = self.patient_selection_state["selected_patient"]
            mobile = patient.get("Details", {}).get("Patient_Mobile") or self.patient_selection_state["mobile_number"]
            appointments = patient.get("Details", {}).get("UpcomingAppointments", [])

            if appointments:
                formatted = ""
                for i, app in enumerate(appointments, 1):
                    date = app.get("Appo_Date", "?")
                    time = app.get("Appo_Period", "?")
                    formatted += f"{i}. {date} - {time}\n"

                if self.current_language == "Arabic":
                    return f"📅 مواعيد المريض {patient.get('TitleAr')}:\n{formatted}"
                else:
                    return f"📅 Appointments for {patient.get('TitleEn')}:\n{formatted}"
            else:
                if self.current_language == "Arabic":
                    return f"⚠️ لا توجد مواعيد حالية للمريض {patient.get('TitleAr')}."
                else:
                    return f"⚠️ No current appointments for {patient.get('TitleEn')}."

        if text.strip() in ["حجز موعد جديد", "book new appointment"] and self.patient_selection_state.get("selected_patient"):
            if self.current_language == "Arabic":
                return "👍 حسنًا، لنبدأ عملية حجز الموعد. اختر العيادة أولًا."
            else:
                return "👍 Okay, let's start booking. Please choose a clinic first."
        ...
        tools = [
                ...
                },
            {
                ...
                "function": {
                    ...
                    "description": "Check if a specific doctor is available for appointments in short details on a specific date must set date and month and day",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "doctor_code": {
